...
class AudioEngine {
  ...
  async initialize() {
    try {
      ...
      console.log('  - masterGainNode.gain:', this.masterGainNode.gain.value);
      console.log('  - micGainNode.gain:', this.micGainNode.gain.value);
      console.log('  - dryGainNode.gain:', this.dryGainNode.gain.value);
      console.log('  - wetGainNode.gain:', this.wetGainNode.gain.value);
      return true;
    } catch (error) {
  ...
  triggerExplosion(pitchShift = 0, reverbAmount = 0.5) {
    if (!this.audioContext) {
      console.error('❌ triggerExplosion: No audioContext!');
      return;
    }

    console.log('💥 AudioEngine.triggerExplosion called');
    console.log('  - audioContext.state:', this.audioContext.state);
    console.log('  - writeIndex:', this.writeIndex, '/ bufferSize:', this.bufferSize);

    // Check if buffer has audio data
    let maxSample = 0;
    for (let i = 0; i < 1000; i++) {
      maxSample = Math.max(maxSample, Math.abs(this.audioBuffer[i]));
    }
    console.log('  - buffer max sample (first 1000):', maxSample);
  ...
  start() {
    if (this.audioContext) {
      console.log('🔊 AudioEngine.start() - context state:', this.audioContext.state);
      if (this.audioContext.state === 'suspended') {
        this.audioContext.resume().then(() => {
          console.log('🔊 AudioContext resumed successfully, state:', this.audioContext.state);
          // Play a quick test tone to confirm audio is working
          this.playTestTone();
        });
      } else if (this.audioContext.state === 'running') {
        // Already running, play test tone
        this.playTestTone();
      }
    }
  }

  // Play a brief test tone to confirm audio output is working
  playTestTone() {
