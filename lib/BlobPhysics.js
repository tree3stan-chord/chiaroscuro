...
class Blob {
  constructor(x, y, radius, hue, bandIndex, bandInfo) {
    ...
    // Spatial movement properties (NEW)
    this.spatialOffsetX = 0;
    this.spatialOffsetY = 0;
    this.spatialPhase = (bandIndex / 24) * Math.PI * 2; // Unique angle per blob

    // Visual properties for audio reactivity
    this.brightness = 60; // Base brightness
    this.saturation = 80; // Base saturation

    // Per-band synthesis state (NEW)
    this.isSynthesizing = false; // Is this blob currently being synthesized?
    this.dragStartX = 0; // Position where drag started
    this.dragStartY = 0;
  ...
  update(energy, width, height, layout = 'arc') {
    ...
    // FIX: Spatial offset system - blobs jump away from home on energy spikes
    // Much stronger force and slower decay for visible movement
    const energyForce = energy * 30; // INCREASED from 8 for dramatic movement
    this.spatialOffsetX += Math.cos(this.spatialPhase) * energyForce;
    this.spatialOffsetY += Math.sin(this.spatialPhase) * energyForce;

    // Drift spatial offset back to zero (slower decay for more visible movement)
    this.spatialOffsetX *= 0.85; // REDUCED from 0.9 for slower return
    this.spatialOffsetY *= 0.85;
    ...
    if (!this.isDragging) {
      ...
      // Update position (base position before spatial offset)
      this.x += this.vx;
      this.y += this.vy;
    ...
    // Audio reactive size with MUCH more variation (increased pulse intensity)
    this.targetRadius = this.baseRadius + energy * 150; // Doubled from 80 for dramatic pulses
    this.smoothRadius += (this.targetRadius - this.smoothRadius) * 0.2; // Faster response
    this.radius = this.smoothRadius;

    // Brightness and saturation modulation (more intense pulses)
    this.brightness = 50 + energy * 40; // 50-90% brightness based on energy
    this.saturation = 70 + energy * 30; // 70-100% saturation based on energy
...
class BlobPhysics {
  ...
  calculateArcPosition(index, total) {
    ...
    const centerY = this.height * 0.6; // Moved up from 0.75 for better visibility
    const radius = Math.min(this.width, this.height) * 0.5; // Wider arc from 0.4

    // Angle from ~160° to ~20° (with margins on both ends)
    const startAngle = Math.PI * 0.9; // ~160°
    const endAngle = Math.PI * 0.1;   // ~20°
