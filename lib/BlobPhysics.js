...
class Blob {
  ...
  update(energy, width, height, layout = 'arc') {
    ...
    // Update position based on layout mode
    if (layout === 'organic') {
      ...
      // Arc and Bar layouts: blobs stay in fixed positions
      this.x = this.homeX;
      this.y = this.homeY;
    }
...
class BlobPhysics {
  ...
  calculateArcPosition(index, total) {
    ...
    const centerY = this.height * 0.85; // Near bottom
  ...
  calculateBarPosition(index, total) {
    ...
    const y = this.height * 0.85; // Near bottom
  ...
  setLayout(newLayout) {
    ...
    this.blobs.forEach((blob, index) => {
      ...
      // In non-organic layouts, snap immediately to new position
      if (newLayout !== 'organic') {
        blob.x = position.x;
        blob.y = position.y;
        blob.vx = 0;
        blob.vy = 0;
