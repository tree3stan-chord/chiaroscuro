...
class Blob {
  ...
  update(energy, width, height, layout = 'arc') {
    ...
    // NEW: Spatial offset system - blobs jump away from home on energy spikes
    // This creates the "breathing" effect independent of position drift
    const energyForce = energy * 8; // Stronger force for more dramatic movement
    this.spatialOffsetX += Math.cos(this.spatialPhase) * energyForce;
    this.spatialOffsetY += Math.sin(this.spatialPhase) * energyForce;

    // Drift spatial offset back to zero (return to home)
    this.spatialOffsetX *= 0.9; // Faster decay for snappier movement
    this.spatialOffsetY *= 0.9;
