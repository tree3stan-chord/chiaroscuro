...
class GenerativeAudioVisualizer {
  ...
  spawnFromAudio() {
    ...
    for (let i = 0; i < this.frequencyBands.length; i++) {
      ...
      if (energy < this.spawnThreshold && transient < 0.1) continue;
      ...
      // Transients spawn bursts
      if (transient > 0.2) {
        this.spawnBurst(spawnX, spawnY, hue, saturation, lightness, transient);
      }

      // Sustained energy spawns particles
      if (energy > this.spawnThreshold && Math.random() < energy * 0.3) {
        this.spawnParticle(spawnX, spawnY, hue, saturation, lightness, energy, angle);
      }

      // High energy spawns blooms
      if (energy > 0.5 && Math.random() < 0.05) {
  ...
  updateParticles() {
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (flow) {
        p.vx += Math.cos(flow.angle) * flow.magnitude * 0.1;
        p.vy += Math.sin(flow.angle) * flow.magnitude * 0.1;
      }
  ...
  handleDrag(x, y, dx, dy) {
    ...
    if (speed > 2) {
      ...
      // Spawn particles along drag path
      for (let i = 0; i < speed * 0.5; i++) {
        const lerpX = x - dx * (i / (speed * 0.5));
        const lerpY = y - dy * (i / (speed * 0.5));
