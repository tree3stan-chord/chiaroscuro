...
class KeyboardSynth {
  ...
  startNote(key) {
    ...
    // Connect: oscillator -> filter -> gain -> dry/reverb
    oscillator.connect(filter);
    filter.connect(gainNode);
    gainNode.connect(this.audioEngine.dryGainNode);
    gainNode.connect(this.audioEngine.reverbNode);
   ...
   * @param {object} params - { pitch, filter, volume }
   */
  modulateNote(key, params = {}) {
    ...
    if (params.filter !== undefined) {
      const filterFreq = 200 + params.filter * 3800; // 200Hz - 4000Hz
      noteData.filter.frequency.setTargetAtTime(filterFreq, now, 0.01);
    }

    // Volume modulation
    if (params.volume !== undefined) {
      const vol = Math.max(0, Math.min(0.5, params.volume));
