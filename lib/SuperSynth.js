...
class SuperSynth {
  ...
  // Frequency-based API (for non-keyboard use)
  noteOn(frequency, velocity = 0.8) {
    if (!this.audioEngine || !this.audioEngine.audioContext) return;

    const ctx = this.audioEngine.audioContext;

    // Create synthetic note info from frequency
    const noteInfo = {
      note: `${frequency.toFixed(1)}Hz`,
      freq: frequency
    };

    // Create voice using frequency
    const voice = new SuperSynthVoice(ctx, noteInfo, this.compressor);
    voice.velocity = velocity;

    // Store by frequency as key
    const key = `freq_${frequency.toFixed(1)}`;
    this.activeVoices.set(key, voice);
    this.currentFreqKey = key;

    return noteInfo;
  }

  noteOff() {
    if (!this.currentFreqKey) return;

    const voice = this.activeVoices.get(this.currentFreqKey);
    if (!voice) return;
    ...
    // Clean up after release
    setTimeout(() => {
      this.activeVoices.delete(this.currentFreqKey);
      this.currentFreqKey = null;
    }, (voice.releaseTime + 0.2) * 1000);
  }

  // Compatibility method (does nothing, already connected in constructor)
  connect(destination) {
    // SuperSynth auto-connects to masterGainNode in constructor
