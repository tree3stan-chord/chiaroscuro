...
class SynthBlob {
   ...
   * @returns {object} { pitch, filter, volume }
   */
  getModulationParams() {
    if (!this.isDragging) {
      return { pitch: 0, filter: 0.5, volume: 0.3 };
    }
    ...
    // Vertical = filter cutoff (0-1)
    const filter = Math.max(0, Math.min(1, 0.5 - (dy / 400)));

    // Volume based on distance (more drag = louder)
    const distance = Math.sqrt(dx * dx + dy * dy);
    const volume = Math.min(0.5, 0.3 + distance / 500);

    return { pitch, filter, volume };
  }
